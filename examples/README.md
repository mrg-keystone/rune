...
Runnable examples for `@mrg-keystone/danet`.
...
> (`@mrg-keystone/danet`, `@danet/core`, `reflect-metadata`) so it reads exactly like a real
> consumer's code. Inside this repo those names are aliased to the local source via the import map
> in `deno.json`. In your own project, add `@mrg-keystone/danet` from JSR and import the same way.
