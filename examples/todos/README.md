...
deno run -A src/bootstrap/mod.ts check examples/todos/src/tasks/tasks.rune

# 2. scaffold / sync the module (idempotent; preserves your filled-in bodies;
#    also writes examples/todos/deno.json — the import map + decorator options)
deno run -A src/bootstrap/mod.ts sync examples/todos/src/tasks/tasks.rune

# 3. prune orphans the spec no longer declares (dev-owned bodies need --force)
deno run -A src/bootstrap/mod.ts sync examples/todos/src/tasks/tasks.rune --force
```
