...
//
// This is the ONE place keywords turn into a grammar. The playground imports
// it for the "Export" buttons; generate.mjs imports it to write files. Both
// share the exact same logic, so the playground preview and the committed
// grammar can never drift.
...
function lineRuleBody(tag) {
  ...
  switch (tag.follows) {
    ...
    default:
      throw new Error(
        `Unknown "follows" kind: ${tag.follows} (tag ${tag.tag})`,
      );
  }
}
...
// Edit the registry (or use the playground) and regenerate.
...
${tagRules}
    ...
    // single colon — distinct from the \`::\` static separator).
    boundary_line: ($) =>
